...
div.w-full.bg-white.border.border-gray-200.rounded-lg.shadow
      div.flex.flex-col.items-center.py-4
            img(:src="imageSrc" alt="User image" @click="clickOnList").w-24.h-24.mb-3.rounded-full.shadow-lg.cursor-pointer
            div.flex
                  button(v-if="isfav" @click="removeFavList").bg-red-500.text-red-50.font-semibold.p-1.border.border-gray-500.rounded.mr-3.my-3.z-20
                  ...
                  button(v-else @click="addFavList" :disabled="isIdxArr" :class="{ Added:isIdxArr}").bg-transparent.text-red-500.font-semibold.p-1.border.border-gray-500.rounded.mr-3.my-3.z-20
                        svg(fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true").w-6.h-6
                              path(stroke-linecap="round" stroke-linejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z")
                  div
