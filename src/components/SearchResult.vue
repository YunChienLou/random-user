<template lang="pug">
div(v-if="isList").grid.grid-cols-5.gap-4.m-4
      Card(v-for="(user,index) in users" :imgUrl="user.picture.thumbnail" :index="index" :name="user.name.first+' '+user.name.last" :gender="user.gender")
div(v-else).grid.grid-cols-1.gap-4.m-4
      List(v-for="(user,index) in users" :imgUrl="user.picture.thumbnail" :index="index" :name="user.name.first+' '+user.name.last" :gender="user.gender")
paginate(
  :page-count="maxPage"
